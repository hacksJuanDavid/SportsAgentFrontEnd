...
import { setListUser } from "@/redux/features/listuser";
...
export default function ListUser() {
  ...
  const usersRedux = useSelector((state: any) => state.listuser.listUser);
  ...
  useEffect(() => {
    dispatch(setListUser(getAllUsers));
  ...
  const handleUpdate = (user: any) => {
    ...
    router.push(`/dashboard/user/update?id=${user.id}`);
  };
  ...
  const handleDelete = (user: any) => {
    // Redirect to update user
    router.push(`/dashboard/user/delete?id=${user.id}`);
  };
